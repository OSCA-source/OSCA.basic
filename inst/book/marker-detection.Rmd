...
- The top rank (`rank.*`) is the smallest rank of each gene across all pairwise comparisons.
...
If a gene has a small top rank, we can conclude that it is one of the top upregulated genes in at least one comparison of $X$ to another cluster.
...
Ranking genes by the top rank is similiar in stringency to ranking by the maximum effect size, in that both will respond to strong DE in a single comparison.
However, the top rank is more useful as it ensures that a single comparison to another cluster with consistently large effects does not dominate the ranking.
If we select all genes with top ranks less than or equal to $T$, the resulting set is the union of the top $T$ genes from all pairwise comparisons.
...
```{r pbmc-grouped-ranked-heat, fig.cap=sprintf("Heatmap of the centered average log-expression values for the top potential marker genes for cluster %s in the PBMC dataset. The set of markers was selected as those genes with top ranks for Cohen's $d$ less than or equal to 5.", target)}
...
To illustrate, we use the smallest rank from `computeTopRank()` to identify the top DE genes in cluster `r target` compared to the other _LYZ_-high clusters (Figure \@ref(fig:pbmc-grouped-ranked-heat2)). 

```{r pbmc-grouped-ranked-heat2, fig.cap=sprintf("Heatmap of the centered average log-expression values for the top potential marker genes for cluster %s relative to other _LYZ_-high clusters in the PBMC dataset. The set of markers was selected as those genes with top ranks for the AUCs.", target)}
...
ordered <- subset[order(computeTopRank(subset)),]
ordered
plotGroupedHeatmap(sce.pbmc[,colLabels(sce.pbmc) %in% lyz.high],
    features=head(rownames(ordered), 20),
    group="label", center=TRUE, zlim=c(-3, 3))
```
