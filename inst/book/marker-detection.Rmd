...
We demonstrate below with the mean AUC for cluster 1 (Figure \@ref(fig:pbmc-scored-markers-1));
the next section will go into more detail on the differences between the various columns.

```{r pbmc-scored-markers-1, fig.height=10, fig.width=6, fig.cap="Distribution of expression values across clusters for the top potential marker genes from cluster 1 in the PBMC dataset."}
chosen <- marker.info[["1"]]
ordered <- chosen[order(chosen$mean.AUC, decreasing=TRUE),1:4]
head(ordered)
...
The example below uses the median Cohen's $d$ to obtain a ranking of upregulated markers for cluster 4 (Figure \@ref(fig:pbmc-scored-markers-again)).
...
```{r pbmc-scored-markers-again, fig.height=10, fig.width=6, fig.cap="Distribution of expression values across clusters for the top potential marker genes from cluster 4 in the PBMC dataset."}
...
When it works, the minimum can be highly effective as it yields a concise set of highly cluster-specific markers. 
...
ordered <- chosen[order(chosen$median.logFC.cohen),]
head(ordered)
```
...
```{r}
demo <- m.out[["1"]] 
demo[order(demo$median.logFC.cohen, decreasing=TRUE),]
```
